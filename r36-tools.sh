...
case "$(uname -s)" in
  Darwin) OS="macos" ;;
  Linux)  OS="linux" ;;
  *)      print "${RED}Unsupported OS: $(uname -s)${R}"; exit 1 ;;
esac

# ── Platform helpers ─────────────────────────────────────────

LOG_FILE="/tmp/r36_$(date +%Y%m%d_%H%M%S).log"
...
file_size() {
    if [[ "$OS" == "macos" ]]; then
        stat -f%z "$1" 2>/dev/null || print 0
    else
        stat -c%s "$1" 2>/dev/null || print 0
    fi
}

# ── Cross-platform disk helpers ──────────────────────────────

list_disks() {
    if [[ "$OS" == "macos" ]]; then
        diskutil list 2>/dev/null | grep "external" | awk '{print $1}' | sed 's|/dev/disk||'
    else
        lsblk -d -o NAME,SIZE,TYPE,MODEL,TRAN -n -p 2>/dev/null | \
            awk -F' ' '{
                name=$1; size=$2; type=$3; model=$4; tran=$5
                if (type == "disk") {
                    if (tran == "usb" || name ~ /mmcblk/) {
                        gsub(/.*\/disk|\/dev\//, "", name)
                        # get just the number at end
                        match(name, /[0-9]+$/)
                        if (RSTART) name = substr(name, RSTART)
                        print name, size, model, tran
                    }
                }
            }'
    fi
...
disk_info() {
    local d=$1 field=$2
    if [[ "$OS" == "macos" ]]; then
        diskutil info "/dev/disk${d}" 2>/dev/null | grep "$field" | cut -d: -f2- | xargs
    else
        # Linux: parse from lsblk
        case "$field" in
            "Disk Size")
                lsblk -d -o NAME,SIZE -n -p 2>/dev/null | awk -v d="$d" '
                    { name=$1; size=$2; gsub(/.*disk|\/dev\//,"",name);
                      match(name, /[0-9]+$/); if (RSTART) name=substr(name,RSTART);
                      if (name == d || name ~ d) print size }'
                ;;
            "Device / Media Name")
                lsblk -d -o NAME,MODEL -n -p 2>/dev/null | awk -v d="$d" '
                    { name=$1; model=$2; gsub(/.*disk|\/dev\//,"",name);
                      match(name, /[0-9]+$/); if (RSTART) name=substr(name,RSTART);
                      if (name == d || name ~ d) print model }'
                ;;
            "Mount Point")
                # For a partition device, get mount point
                target="$d"
                if [[ -b "$target" ]]; then
                    findmnt -n -o TARGET "$target" 2>/dev/null
                fi
                ;;
            *)
                print ""
...
find_mount_point() {
    local dev="$1"
    if [[ "$OS" == "macos" ]]; then
        diskutil info "$dev" 2>/dev/null | grep "Mount Point" | cut -d: -f2 | xargs
    else
        findmnt -n -o TARGET -S "$dev" 2>/dev/null || print ""
    fi
}
...
unmount_disk() {
    ...
    if [[ "$OS" == "macos" ]]; then
        ...
        local parts
        parts=$(lsblk -o NAME -n "$dev" 2>/dev/null | tail -n +2)
        for p in $parts; do
            sudo umount "/dev/$p" 2>/dev/null
        done
    fi
}
...
disk_available() {
    local dev="$1"
    if [[ "$OS" == "macos" ]]; then
        diskutil list "$dev" >/dev/null 2>&1
    else
        [[ -b "$dev" ]]
    fi
}
...
select_disk_push() {
    ...
    if [[ "$OS" == "macos" ]]; then
        ...
        lsblk -d -o NAME,SIZE,TYPE,MODEL,TRAN -n -p 2>/dev/null | \
            grep -E "disk" | while read -r line; do
            local name devname
            name=$(echo "$line" | awk '{print $1}')
            devname=$(basename "$name")
            # Extract trailing number from device name
            local num
            num=$(echo "$devname" | sed 's/[^0-9]*//g')
            [[ -z "$num" ]] && num=$(echo "$devname" | sed 's/[a-z]//g')
            [[ -z "$num" ]] && num=0
            # Filter for removable
            local tran
            tran=$(echo "$line" | awk '{print $5}')
            if [[ "$tran" == "usb" || "$devname" == mmcblk* || "$devname" == sd* ]]; then
                print "$num" >> "$tmpfile"
            fi
        done
    fi
    ...
    while IFS= read -r num; do
        [[ -z "$num" ]] && continue
        local size_human model
        if [[ "$OS" == "macos" ]]; then
            size_human=$(diskutil info "/dev/disk${num}" 2>/dev/null | grep "Disk Size" | awk '{print $3, $4}')
            model=$(diskutil info "/dev/disk${num}" 2>/dev/null | grep "Device / Media Name" | cut -d: -f2 | xargs)
            print "  ${GREEN}[$(( ++count ))]${R} /dev/disk${num}  ${BOLD}$size_human${R}  ${DIM}$model${R}"
        else
            local devpath
            devpath=$(lsblk -d -o NAME -n -p 2>/dev/null | grep -E "[0-9]*$num" | head -1)
            [[ -z "$devpath" ]] && devpath="/dev/sd${num}"
            size_human=$(lsblk -d -o SIZE -n "$devpath" 2>/dev/null || print "?")
            model=$(lsblk -d -o MODEL -n "$devpath" 2>/dev/null | xargs || print "?")
            print "  ${GREEN}[$(( ++count ))]${R} $devpath  ${BOLD}$size_human${R}  ${DIM}$model${R}"
        fi
        nums+=("$num")
    done < "$tmpfile"
    rm -f "$tmpfile"
    ...
    if (( count == 1 )); then
        ...
        if [[ "$OS" == "macos" ]]; then
            ...
            DISK=$(lsblk -d -o NAME -n -p 2>/dev/null | grep -E "[0-9]*$DISK_NUM" | head -1)
            [[ -z "$DISK" ]] && DISK="/dev/sd${DISK_NUM}"
            print "  ${CYAN}Auto-selected: $DISK${R}"
        fi
    else
...
select_partition() {
    ...
    if [[ "$OS" == "macos" ]]; then
        ...
        printf "  ${BOLD}Partition name (e.g. sdb1, mmcblk0p1): ${R}"
        read -r PARTITION
        [[ -z "$PARTITION" ]] && { print "  ${RED}No partition selected.${R}"; exit 1 }
        local pdev="/dev/$PARTITION"
        [[ ! -b "$pdev" ]] && { print "  ${RED}${pdev} not found.${R}"; exit 1 }
    fi
}
...
mount_partition() {
    ...
    local mount_pt
    mount_pt=$(find_mount_point "$target")
...
remount() {
    ...
    sleep 2
    local new_mp
    new_mp=$(find_mount_point "$target")
...
select_disk_write() {
    ...
    if [[ "$OS" == "macos" ]]; then
        ...
        lsblk -d -o NAME,SIZE,TYPE,MODEL,TRAN -n -p 2>/dev/null | \
            grep -E "disk" | while read -r line; do
            local name devname tran
            name=$(echo "$line" | awk '{print $1}')
            devname=$(basename "$name")
            trans=$(echo "$line" | awk '{print $5}')
            if [[ "$trans" == "usb" || "$devname" == mmcblk* || "$devname" == sd* ]]; then
                print "$name" >> "$tmpfile"
            fi
        done
    fi
    ...
    while IFS= read -r line; do
        ...
        if [[ "$OS" == "macos" ]]; then
            ...
            local devpath="$line"
            size=$(lsblk -d -o SIZE -n "$devpath" 2>/dev/null || print "?")
            model=$(lsblk -d -o MODEL -n "$devpath" 2>/dev/null | xargs || print "?")
            print "  ${GREEN}[$count]${R} $devpath  ${BOLD}$size${R}  ${DIM}$model${R}"
            nums+=("$devpath")
        fi
    done < "$tmpfile"
    rm -f "$tmpfile"
    ...
    if (( count == 1 )); then
        local sel="${nums[1]}"
        print "  ${CYAN}Auto-selected: $sel${R}"
    else
        printf "  ${BOLD}Select [1-%d]: ${R}" $count
        read -r choice
...
offer_verify() {
    ...
    local ih ch
    ih=$(dd if="$IMG" bs=1m count=512 2>/dev/null | md5_hash)
    ch=$(dd if="$RDISK" bs=1m count=512 2>/dev/null | md5_hash)
    if [[ "$ih" == "$ch" ]]; then
...
# ═══════════════════════════════════════════════════════════════
#  TOOL 3: encode  — re-encode media for R36, push to card
# ═══════════════════════════════════════════════════════════════
...
tool_encode() {
    ...
    local out_bytes
    out_bytes=$(file_size "$OUTPUT_FILE")
    local human
